...
class Login extends React.Component {
  ...
  tick() {
    let secondsElapsed = this.state.secondsElapsed - 1;
    if (secondsElapsed == 0) {
  ...
  getCode() {
    ...
    let thiz = this;
    Util.post(API.getSmsCode(), {tel: phone, type: 'verifiycode'}, function(
      ret,
    ) {
      if (ret.code == 0) {
  ...
  login() {
    ...
    fetch(
      API.LOGIN + '?user_name=' + phone + '&code=' + code + '&type=verifiycode',
    )
  ...
  renderLogined() {
    return (
      <View style={[styles.container]}>
        ...
        <TouchableHighlight
          style={[styles.btn, styles.marginTop30]}
          onPress={this.logout}>
  ...
  renderLogin() {
    let getCode_text =
      this.state.secondsElapsed == 0
        ? '获取验证码'
        : this.state.secondsElapsed + '秒后重试';

    return (
      <Image
        style={[Util.size, styles.container]}
        source={require('image!bg_login')}>
        <View style={styles.loginform}>
          ...
          <View style={[styles.inputRow, {marginTop: 10}]}>
            ...
            />
            <TouchableHighlight
              style={[styles.btn, {width: 80, height: 30}]}
